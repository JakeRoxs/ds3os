...
BOOL(WINAPI*
         Real_FreeLibrary)(HMODULE a0) = FreeLibrary;
...
HMODULE(WINAPI*
            Real_GetModuleHandleW)(LPCWSTR a0) = GetModuleHandleW;
...
HMODULE(WINAPI*
            Real_LoadLibraryW)(LPCWSTR a0) = LoadLibraryW;
...
BOOL ImportEnumerate(HINSTANCE hInst) {
  ...
  nSectionsOffset = nPeOffset + sizeof(pNtHeader->Signature) + sizeof(pNtHeader->FileHeader) + pNtHeader->FileHeader.SizeOfOptionalHeader;
  ...
  DWORD rvaImageDirectory = pNtHeader->OptionalHeader
                                ...
                                .VirtualAddress;
  PIMAGE_IMPORT_DESCRIPTOR iidp = (PIMAGE_IMPORT_DESCRIPTOR)RvaToVa(pbBase, rvaImageDirectory);
  ...
  for (DWORD n = 0; n < nFiles; n++, iidp++) {
    ...
    for (DWORD f = 0; f < nNames; f++) {
      ...
      if (rvaName & IMAGE_ORDINAL_FLAG) {
        ...
        PIMAGE_IMPORT_BY_NAME pName = (PIMAGE_IMPORT_BY_NAME)RvaToVa(pbBase, rvaName);
        if (pName) {
