...
//////////////////////////////////////////////////////////////////////////////
//
int(WINAPI* Real_EntryPoint)(VOID) = NULL;
...
BOOL(WINAPI* Real_CloseHandle)(HANDLE a0) = CloseHandle;
...
BOOL(WINAPI* Real_DeleteFileW)(LPCWSTR a0) = DeleteFileW;
...
DWORD(WINAPI* Real_GetFileAttributesW)(LPCWSTR a0) = GetFileAttributesW;
...
HMODULE(WINAPI* Real_LoadLibraryA)(LPCSTR a0) = LoadLibraryA;

HMODULE(WINAPI* Real_LoadLibraryW)(LPCWSTR a0) = LoadLibraryW;
...
VOID(WINAPI* Real_ExitProcess)(UINT a0) = ExitProcess;

DWORD(WINAPI* Real_ExpandEnvironmentStringsA)(PCSTR lpSrc, PCHAR lpDst, DWORD nSize) = ExpandEnvironmentStringsA;

DWORD(WINAPI* Real_ExpandEnvironmentStringsW)(PCWSTR lpSrc, PWCHAR lpDst, DWORD nSize) = ExpandEnvironmentStringsW;

DWORD(WINAPI* Real_GetEnvironmentVariableA)(PCSTR lpName, PCHAR lpBuffer, DWORD nSize) = GetEnvironmentVariableA;

DWORD(WINAPI* Real_GetEnvironmentVariableW)(PCWSTR lpName, PWCHAR lpBuffer, DWORD nSize) = GetEnvironmentVariableW;
...
BOOL CreateProcessInternals(HANDLE hProcess, DWORD nProcessId, PCHAR pszId,
  ...
  s_ChildPayload.rGeneology[s_ChildPayload.nGeneology] = (DWORD)InterlockedIncrement(&s_nChildCnt);
...
LONG DetachDetours(VOID) {
  ...
  if (Real_getenv) {
    DetourDetach(&(PVOID&)Real_getenv, Mine_getenv);
  }
  if (Real_getenv_s) {
    DetourDetach(&(PVOID&)Real_getenv_s, Mine_getenv_s);
  }
  if (Real_wgetenv) {
    DetourDetach(&(PVOID&)Real_wgetenv, Mine_wgetenv);
  }
  if (Real_wgetenv_s) {
    DetourDetach(&(PVOID&)Real_wgetenv, Mine_wgetenv_s);
  }
  if (Real_dupenv_s) {
    DetourDetach(&(PVOID&)Real_dupenv_s, Mine_dupenv_s);
  }
  if (Real_wdupenv_s) {
    DetourDetach(&(PVOID&)Real_wdupenv_s, Mine_wdupenv_s);
  }
