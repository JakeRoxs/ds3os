...
enum EShaderSection {
  kGLMARBVertex,
  kGLMARBVertexDisabled,
  kGLMARBFragment,
  kGLMARBFragmentDisabled,
  kGLMGLSLVertex,
  kGLMGLSLVertexDisabled,
  kGLMGLSLFragment,
  kGLMGLSLFragmentDisabled,
...
void CGLMProgram::SetProgramText(char* text) {
  ...
  for (int i = 0; i < sectionCount; i++) {
    ...
    case kGLMVertexProgram:
      ...
      default:
        Assert(!"Mismatched section marker seen in SetProgramText (VP)");
        break;
      }
      break;
...
void CGLMShaderPairCache::HashRowProbe(CGLMPairCacheEntry* row, CGLMProgram* vp, CGLMProgram* fp, uint extraKeyBits, int* hitwayOut, int* emptywayOut, int* oldestwayOut) {
  ...
  if (hitwayOut)
    *hitwayOut = hitway;
  if (emptywayOut)
    *emptywayOut = emptyway;
  if (oldestwayOut)
    *oldestwayOut = oldestway;
}
