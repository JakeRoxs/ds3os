...
void header(FILE* outf, const int type, const unsigned long blen, const unsigned long klen) {
  ...
  switch (type) {
  case 0:
    fprintf(outf, "Variable Key Known Answer Tests");
    break;
  case 2:
    fprintf(outf, "Variable Key Known Answer Tests");
    break;
  case 1:
    fprintf(outf, "Variable Text Known Answer Tests");
    break;
  case 3:
    fprintf(outf, "Variable Text Known Answer Tests");
    break;
  case 4:
  case 6:
    fprintf(outf, "Monte Carlo (Encryption) Tests");
    break;
  case 5:
  case 7:
    fprintf(outf, "Monte Carlo (Decryption) Tests");
    break;
  }
...
void comp_vecs(const char* fn1, const char* fn2) {
  ...
  if (fopen_s(&if1, fn1, "r")) {
    printf("\n*** 1st file (%s) not found ***", fn1);
    return;
  }

  if (fopen_s(&if2, fn2, "r")) {
    printf("\n*** 2nd file (%s) not found ***", fn2);
    return;
  }
  ...
  for (;;) // while there is still input
  {
    ...
    {
      ty1 = find_line(if1, str1);
      ty2 = find_line(if2, str2);
    }
    ...
    if (cmp_nocase(str1, str2) == 0) {
      req = TRUE;
      continue; // if lines are the same continue
    }
