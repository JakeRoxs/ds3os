...
void do_test(const char* in_dir, const char* out_dir, const char* name, int gen_flag) {
  std::string line;
#if OMAC_VERSION == 1
  std::string f_name("OMAC1");
#else
  std::string f_name("OMAC2");
#endif
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      omac_init(key, key_len, contx);
      omac_data((unsigned char*)msg, msg_len, contx);
      omac_end(buf, contx);

      if (memcmp(buf, tag, tag_len)) {
        std::cout << std::endl
                  << "\tauthentication error on test number " << vec_no;
        err++;
      }
      delete[] buf;
      if (gen_flag & !err)
    ...
    if (v_err == 0)
      ...
      std::cout << std::endl
                << v_err << " errors in " << vec_no << " vectors";
    else
...
int main(int argc, char* argv[]) {
  if (argc == 4)
    ...
    std::cout << std::endl
              << "usage: input_directory output_directory mode_name";
  std::cout << std::endl;
  ...
  omac_init(key, 16, ctx);
  omac_data(data, 1024, ctx);
  omac_end(tag0, ctx);
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    uint8_t* dp = data;
    for (int i = 0; i < 32; ++i) {
      omac_data(dp, rpos[i], ctx);
      dp += rpos[i];
    }
    omac_data(dp, data + 1024 - dp, ctx);
    omac_end(tag, ctx);
  ...
  if (err == 0)
    std::cout << "Incremental message authntication passed" << std::endl
              << std::endl;
