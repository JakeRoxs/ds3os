...
#define mlen_len(x)                                                \
  (((x) & 0xff00000000000000) ? 7 : ((x) & 0xffff000000000000) ? 6 \
                                : ((x) & 0xffffff0000000000)   ? 5 \
                                : ((x) & 0xffffffff00000000)   ? 4 \
                                : ((x) & 0xffffffffff000000)   ? 3 \
                                : ((x) & 0xffffffffffff0000)   ? 2 \
                                                               : 1)

#else
#define mlen_len(x) (((x) & 0xff000000) ? 3 : ((x) & 0xffff0000) ? 2 \
                                                                 : 1)
#endif
                          ...
                          ccm_ctx ctx[1])           /* the mode context             */
{
  ...
  UI8_PTR(ctx->ctr_val)
  [BLOCK_SIZE - 1] = 0;                /* set initial counter      */
  set_ctr(ctx->cbc_buf, ctx->msg_len); /* store the message length */

  UI8_PTR(ctx->cbc_buf)
  [0] |= (ctx->hdr_lim ? 0x40 : 0) + (ctx->tag_len ? (ctx->tag_len - 2) << 2 : 0);
  /* encrypt the cbc block    */
  aes_encrypt(UI8_PTR(ctx->cbc_buf), UI8_PTR(ctx->cbc_buf), ctx->aes);

  if (ctx->hdr_len) { /* encode the length field if there is some associated data */

    if (ctx->hdr_len < 65536 - 256) {
      UI8_PTR(ctx->cbc_buf)
      [0] ^= (uint8_t)(ctx->hdr_lim >> 8);
      UI8_PTR(ctx->cbc_buf)
      [1] ^= (uint8_t)ctx->hdr_lim;
      ctx->cnt = 2;
    }
      ...
      [1] ^= 0xfe;
      UI8_PTR(ctx->cbc_buf)
      [2] ^= (uint8_t)(ctx->hdr_lim >> 24);
      UI8_PTR(ctx->cbc_buf)
      [3] ^= (uint8_t)(ctx->hdr_lim >> 16);
      UI8_PTR(ctx->cbc_buf)
      [4] ^= (uint8_t)(ctx->hdr_lim >> 8);
      UI8_PTR(ctx->cbc_buf)
      [5] ^= (uint8_t)ctx->hdr_lim;
      ctx->cnt = 6;
    } else {
      UI8_PTR(ctx->cbc_buf)
      [0] ^= 0xff;
      ...
      [1] ^= 0xff;
      UI8_PTR(ctx->cbc_buf)
      [2] ^= (uint8_t)(ctx->hdr_lim >> 56);
      UI8_PTR(ctx->cbc_buf)
      [3] ^= (uint8_t)(ctx->hdr_lim >> 48);
      UI8_PTR(ctx->cbc_buf)
      [4] ^= (uint8_t)(ctx->hdr_lim >> 40);
      UI8_PTR(ctx->cbc_buf)
      [5] ^= (uint8_t)(ctx->hdr_lim >> 32);
      UI8_PTR(ctx->cbc_buf)
      [6] ^= (uint8_t)(ctx->hdr_lim >> 24);
      UI8_PTR(ctx->cbc_buf)
      [7] ^= (uint8_t)(ctx->hdr_lim >> 16);
      UI8_PTR(ctx->cbc_buf)
      [8] ^= (uint8_t)(ctx->hdr_lim >> 8);
      UI8_PTR(ctx->cbc_buf)
      [9] ^= (uint8_t)ctx->hdr_lim;
      ctx->cnt = 10;
    }
#endif
    ctx->hdr_lim += ctx->cnt;
                         ...
                         ccm_ctx ctx[1])        /* the mode context             */
{
  ...
  if (tag_len != ctx->tag_len || ctx->state == INIT_STATE && ctx->hdr_lim || ctx->state == AUTH_STATE)
    return (ctx->state = CCM_AUTH_LENGTH_ERROR);

  if (ctx->txt_acnt != ctx->cnt && ctx->cnt > 0)
    return RETURN_ERROR;

  if (ctx->cnt < ctx->msg_len || ctx->cnt + tag_len > ctx->msg_len + ctx->tag_len)
    return (ctx->state = CCM_MSG_LENGTH_ERROR);
