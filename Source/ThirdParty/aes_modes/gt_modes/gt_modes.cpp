...
void do_test(unsigned int ntests, enum mode mm) {
  ...
  for (i = 0; i < ntests; ++i) {
    ...
    if (memcmp(ptx, res[0], msg_len)) {
      std::cout << std::endl
                << mode_name[mm] << ": (BRG) encryption or decryption error";
      err1 = RETURN_ERROR;
    }
    if (err != RETURN_GOOD) {
      std::cout << std::endl
                << mode_name[mm] << ": (BRG) tag error";
      err1 = RETURN_ERROR;
    }
    if (memcmp(ptx, res[1], msg_len)) {
      std::cout << std::endl
                << mode_name[mm] << ": (LTC) encryption or decryption error";
      err1 = RETURN_ERROR;
    }
    if (stat != RETURN_GOOD) {
      std::cout << std::endl
                << mode_name[mm] << ": (LTC)tag error";
      err1 = RETURN_ERROR;
    }
    if (memcmp(ctx[0], ctx[1], msg_len)) {
      std::cout << std::endl
                << mode_name[mm] << ": ciphertext mismatch error";
      err1 = RETURN_ERROR;
    }
    if (memcmp(tag[0], tag[1], AES_BLOCK_SIZE)) {
      std::cout << std::endl
                << mode_name[mm] << ": tag mismatch error";
      err1 = RETURN_ERROR;
    }
  }
  if (err1 == RETURN_GOOD) {
    std::cout << std::endl
              << mode_name[mm] << ": (BRG) and (LTC) outputs match";
  }
}
...
int gen_vectors(char* in_file_name, char* out_file_name) {
  ...
  while (!in_file.eof()) {
    ...
    } while (!in_file.eof() && strncmp((char*)line.c_str(), "MODE", 4) != 0);
    ...
    out_file << std::endl
             << "REM Produced by GENTEST on " << buf;
    ...
    while (!in_file.eof()) {
      ...
      do {
        ...
        if (ea_funs.brg_ifun[mm](v_key, (unsigned long)l_key, context) != RETURN_GOOD) {
          std::cout << std::endl
                    << "Initialisation failure in BRG's " << mode_name[mm] << " mode code";
          goto exit1;
        }
        ...
        if (ea_funs.brg_efun[mm](
                ...
                context) != RETURN_GOOD) {
          std::cout << std::endl
                    << "Encryption failure in BRG's " << mode_name[mm] << " mode code";
          goto exit1;
        }
        ...
        if (ea_funs.ltc_fun[mm](
          ...
          std::cout << std::endl
                    << "Failure in LTC's " << mode_name[mm] << "mode code";
          goto exit1;
        }
        ...
        if (memcmp(ctx[0], ctx[1], l_ptx)) {
          std::cout << std::endl
                    << "Ciphertext mismatch for " << mode_name[mm] << "mode on vector" << (int)vec_no;
          goto exit1;
        }
        ...
        if (memcmp(tag[0], tag[1], *(v_tgl))) {
          std::cout << std::endl
                    << "Tag mismatch for " << mode_name[mm] << " mode on vector " << (int)vec_no;
          goto exit1;
        }
...
int main(int argc, char* argv[]) {
  ...
  if (argc == 3)
    ...
    std::cout << std::endl
              << "usage: gentest vector_rule_file vector_output_file" << std::endl;
#endif
  std::cout << std::endl
            << std::endl;
  return 0;
