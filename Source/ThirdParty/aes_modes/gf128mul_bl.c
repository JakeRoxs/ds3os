/*
 ...
 BL (see gfmul128.h) with the following in memory layout.
...
#define CONVERT (REVERSE_BITS | REVERSE_BYTES)
#endif
...
void init_64k_table(gf_t g, gf_t64k_t t) {
  ...
  /*
  the byte value 0x01 at the lowest byte position in a[]
  is unity in this field representation g[] goes into
  this position in the table. 0x02 corresponds to a field
  value of 2 so we can determine this value by multiplying
  the 0x01 value by x, a process we can repeat for 8 field
  values.
  */
#ifdef CHANGE_GF_REPRESENTATION
  convert_representation(t[0][1], g, CONVERT);
#else
  memcpy(t[0][1], g, GF_BYTE_LEN);
#endif
  memset(t[0][0], 0, GF_BYTE_LEN);
  for (j = 1; j <= 64; j <<= 1)
    gf_mulx1_bl(t[0][j + j], t[0][j]);
...
#define xor_64k(i, ap, t, r) xor_block_aligned(r, r, t[i][ap[15 - i]])
...
void init_8k_table(gf_t g, gf_t8k_t t) {
  ...
  */
  ...
  /*  now do the high nibble: g * {x^4} = x * g * {x^3} */
  gf_mulx1_bl(t[1][1], t[0][8]);
  for (j = 1; j <= 4; j <<= 1)
    gf_mulx1_bl(t[1][j + j], t[1][j]);
...
#define xor_8k(i, ap, t, r)                           \
  xor_block_aligned(r, r, t[i + i][ap[15 - i] & 15]); \
  xor_block_aligned(r, r, t[i + i + 1][ap[15 - i] >> 4])
...
#ifdef CHANGE_GF_REPRESENTATION
  convert_representation(t[1], g, CONVERT);
#else
  memcpy(t[1], g, GF_BYTE_LEN);
#endif
  memset(t[0], 0, GF_BYTE_LEN);
  for (j = 1; j <= 64; j <<= 1)
    gf_mulx1_bl(t[j + j], t[j]);
...
#define xor_4k(i, ap, t, r) \
  gf_mulx8_bl(r);           \
  xor_block_aligned(r, r, t[ap[15 - i]])
...
#define xor_256(i, ap, t, r)                   \
  gf_mulx4_bl(r);                              \
  xor_block_aligned(r, r, t[ap[15 - i] >> 4]); \
  gf_mulx4_bl(r);                              \
  xor_block_aligned(r, r, t[ap[15 - i] & 15])
