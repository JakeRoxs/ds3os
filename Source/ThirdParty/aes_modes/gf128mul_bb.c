...
void init_8k_table(gf_t g, gf_t8k_t t) {
  ...
  */
  ...
  for (j = 4; j > 0; j >>= 1)
    gf_mulx_lb(t[1][j], t[1][j + j]);

  /*  now do the high nibble: g * {x^4} = x * g * {x^3} */
  gf_mulx1_bb(t[0][8], t[1][1]);
  for (j = 4; j > 0; j >>= 1)
    gf_mulx1_bb(t[0][j], t[0][j + j]);
...
#define xor_256(i, ap, t, r)                     \
  gf_mulx4_bb(r);                                \
  xor_block_aligned(r, r, t[ap[15 - i] & 0x0f]); \
  gf_mulx4_bb(r);                                \
