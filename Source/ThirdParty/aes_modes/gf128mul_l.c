...
/* choose the Galois Field representation to use (see gfmul128.h) */
#if 0
#define GF_MODE_LL
#define mode _ll
#elif 0
...
/*  This version uses 64k bytes of table space on the stack.
    ...
    mulltiplying g by the field value in question. We can
    then combi9ne these values to provide the full multiply.
    So for each of 16 bytes we have a table of 256 field
    values each of 16 bytes - 64k bytes in total.
*/
...
#define xor_4k(i, ap, t, r) \
  gf_mulx8(mode)(r);        \
  xor_block_aligned(r, r, t[ap[GF_INDEX(i)]])
...
#define x_lo(i, ap, t, r) \
  gf_mulx4(mode)(r);      \
  xor_block_aligned(r, r, t[ap[GF_INDEX(i)] & 0x0f])
#define x_hi(i, ap, t, r) \
  gf_mulx4(mode)(r);      \
  xor_block_aligned(r, r, t[ap[GF_INDEX(i)] >> 4])

#if defined(GF_LB) || defined(GF_BB)
