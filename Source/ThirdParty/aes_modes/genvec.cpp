...
int v_rec::get_value(byte_array& arr) {
  ...
  if (*p) {
    if (*p == '=') { /* use previous ciphertext */
      ++p;
      retv = 1 | ctx_copy;
    } else if (isxdigit(*p) && isxdigit(*(p + 1))) { /* input a hexadecimal byte sequence */
      while (hexbyte_in(p, (unsigned char*)chr)) {
        arr.push_back(chr[0]);
        p += 2;
      }
      retv = 1;
    } else if (*p == '#') { /* input a decimal value and assemble into  */
