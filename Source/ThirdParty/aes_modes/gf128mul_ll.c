...
void init_8k_table(gf_t g, gf_t8k_t t) {
  ...
  */
  ...
  gf_mulx1_ll(t[1][1], t[0][8]);
  for (j = 1; j <= 4; j <<= 1)
    gf_mulx1_ll(t[1][j + j], t[1][j]);
...
#define xor_256(i, ap, t, r)              \
  gf_mulx4_ll(r);                         \
  xor_block_aligned(r, r, t[ap[i] >> 4]); \
  gf_mulx4_ll(r);                         \
  xor_block_aligned(r, r, t[ap[i] & 0x0f])
