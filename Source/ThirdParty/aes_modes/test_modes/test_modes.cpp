  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (ptx_rpt == 1 && memcmp(enc_buf, ctx, ptx_len)) {
        std::cout << std::endl
                  << "\tencrypt ciphertext error on test number " << vec_no;
        err++;
      }
      delete[] enc_buf;
      ...
      if (gen_flag)
        ...
        std::cout << std::endl
                  << "\tencrypt tag error on test number " << vec_no;
        err++;
      }
      ...
      if (ptx_rpt == 1 && memcmp(dec_buf, ptx, ptx_len)) {
        std::cout << std::endl
                  << "\tdecrypt plaintext error on test number " << vec_no;
        err++;
      }
      delete[] dec_buf;
      ...
      if (memcmp(dec_tag, tag, tag_len)) {
        std::cout << std::endl
                  << "\tdecrypt tag error on test number " << vec_no;
        err++;
      }
      delete[] enc_tag;
      delete[] dec_tag;
